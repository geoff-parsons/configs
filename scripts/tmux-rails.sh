...
tmux send-keys -t $NAME:3 "rvm use $RVM && clear && rails server" C-m

tmux new-window -dt $NAME:4 -n "console"
tmux send-keys -t $NAME:4 "rvm use $RVM && clear && rails console" C-m

tmux new-window -dt $NAME:5 -n "shell"
tmux send-keys -t $NAME:5 "rvm use $RVM; clear" C-m
